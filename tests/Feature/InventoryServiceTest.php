...
class InventoryServiceTest extends TestCase {
 ...
 public function test_rejects_non_stackable_inactive_and_hidden_items(){ foreach([['is_stackable'=>false,'status'=>'active'],['is_stackable'=>true,'status'=>'inactive'],['is_stackable'=>true,'status'=>'hidden']] as $i=>$state){$item=Item::create(['code'=>'bad_'.$i,'name'=>'Bad','item_type'=>'material','rarity'=>'common','max_stack'=>$state['is_stackable']?99:1]+$state);try{$this->service->addItem($this->character,$item,1);$this->fail();}catch(InvalidArgumentException $e){$this->assertTrue(true);}} }
