...
class AddInventoryItemCommandTest extends TestCase {
 ...
 public function test_command_fails_for_non_stackable_item(){ $character=$this->character();$item=Item::create(['code'=>'unique_item','name'=>'Unique','item_type'=>'equipment','rarity'=>'common','is_stackable'=>false,'max_stack'=>null,'status'=>'active']);$this->artisan('game:inventory:add',['characterId'=>$character->id,'itemCode'=>$item->code,'quantity'=>1])->assertExitCode(1); }
