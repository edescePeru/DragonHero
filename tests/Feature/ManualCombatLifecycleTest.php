...
class ManualCombatLifecycleTest extends TestCase
{
    ...
    public function test_hunting_heartbeat_is_blocked_until_abandon_or_expiration_releases_slot()
    {
        ...
        $this->abandon($character,$combat)->assertOk();app(HuntingSessionService::class)->tick($character,$hunting->fresh());$this->assertGreaterThan($before,Hunt::count());
        $other=$this->character();list($otherHunting,$otherCombat)=$this->context($other);$otherBefore=Hunt::count();$this->stale($otherCombat);$this->actingAs($other->user)->getJson(route('characters.manual-combats.show',[$other,$otherCombat]))->assertOk()->assertJsonPath('status',ManualCombatStatus::EXPIRED);app(HuntingSessionService::class)->tick($other,$otherHunting->fresh());$this->assertGreaterThan($otherBefore,Hunt::count());
    }
