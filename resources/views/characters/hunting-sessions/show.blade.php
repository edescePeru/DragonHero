...
@section('content')
<div id="hunting-session" data-tick-url="{{ route('characters.hunting-sessions.tick',[$character,$session['session_id']]) }}">
    <h1 class="fs-3">Cacería conectada</h1>
...
(function(){
    ...
    let timer=null,requestInFlight=false,lastState=@json($session),lastRenderedProcessedHuntId=null,playback=null,playbackFrame=null;
    ...
    function prependCombatLog(node){const scroll=document.getElementById('combat-log-scroll');const wasAtTop=scroll.scrollTop<=20;document.getElementById('combat-log').prepend(node);if(wasAtTop)scroll.scrollTop=0;else document.getElementById('combat-log-new-events').classList.remove('d-none');}
    function appendEventLog(event,names){const actor=names.get(event.actor_identifier);const target=names.get(event.target_identifier);const actorName=actor?actor.display_name:event.actor_identifier;const targetName=target?target.display_name:event.target_identifier;let message,modifier='';if(!event.did_hit){message='FALLO · '+actorName+' atacó a '+targetName+', pero falló.';modifier='miss';}else if(event.is_critical){message='CRÍTICO · '+actorName+' hizo '+event.damage+' de daño a '+targetName+'.';modifier='critical';}else message=actorName+' atacó a '+targetName+' e hizo '+event.damage+' de daño.';prependCombatLog(combatLogEntry(message,modifier));if(Number(event.target_health_before)>0&&Number(event.target_health_after)===0)prependCombatLog(combatLogEntry(targetName+' fue derrotado.','defeat'));}
    ...
    function startPlayback(hunt,reward,serverTime){if(!hunt)return;if(playback&&Number(playback.hunt.hunt_id)===Number(hunt.hunt_id))return;if(playbackFrame!==null)cancelAnimationFrame(playbackFrame);clearNode(document.getElementById('combat-log'));document.getElementById('combat-log-new-events').classList.add('d-none');const elapsed=Math.max(0,Date.parse(serverTime)-Date.parse(hunt.resolved_at));playback={hunt:hunt,reward:reward||hunt.historical_reward,initialElapsed:elapsed,startedPerformance:performance.now(),nodes:buildParticipants(hunt),names:participantMap(hunt),renderedSequences:new Set(),resultRendered:false,lootRendered:false,initialRender:true};if(!hunt.events||hunt.events.length===0){renderPlaybackAt(playback,Number(hunt.playback_duration_ms));return;}playbackLoop();}
    function skipPlayback(){if(!playback)return;if(playbackFrame!==null){cancelAnimationFrame(playbackFrame);playbackFrame=null;}renderPlaybackAt(playback,Number(playback.hunt.playback_duration_ms));}
    function render(state){
        ...
        document.querySelector('#stop-form button').disabled=state.status==='stopped';
    }
    function schedule(delay){clearTimer();if(delay!==null)timer=setTimeout(tick,delay);}
    async function tick(){
        if(requestInFlight)return;requestInFlight=true;clearTimer();
        try{const response=await fetch(root.dataset.tickUrl,{method:'POST',headers:{'X-CSRF-TOKEN':token,'Accept':'application/json','X-Requested-With':'XMLHttpRequest'}});if(!response.ok)throw new Error();const state=await response.json();render(state);schedule(delayFor(state));}
        catch(error){schedule(5000);}
        finally{requestInFlight=false;}
    }
    document.getElementById('skip-playback').addEventListener('click',skipPlayback);
