...
(function(){
    ...
    function renderGeneratedReward(processedHunt,reward,isNewProcessedHunt){
        ...
        content.appendChild(paragraph('Oro pendiente: '+reward.gold_amount+' · Experiencia pendiente: '+reward.experience_amount));
    ...
    function renderRewardsSummary(summary,contentId){
        ...
        content.appendChild(paragraph('Oro pendiente: '+summary.gold_amount+' · Experiencia pendiente: '+summary.experience_amount));
    ...
    function renderInventoryItems(stackables,instances,status){const content=document.getElementById('inventory-items');clearNode(content);document.getElementById('inventory-used-slots').textContent=status.current_used_slots;document.getElementById('inventory-free-slots').textContent=status.current_free_slots;document.getElementById('inventory-effective-capacity').textContent=status.effective_capacity;if((!stackables||stackables.length===0)&&(!instances||instances.length===0)){content.appendChild(paragraph('Tu inventario está vacío'));return;}(stackables||[]).forEach(function(item){const row=document.createElement('div');row.className='d-flex justify-content-between border-bottom py-2';const identity=document.createElement('span');identity.textContent=item.item_name+' · '+item.item_code;const quantity=document.createElement('strong');quantity.textContent='x'+item.quantity+' · '+item.used_slots+' slots · stack '+item.max_stack;row.appendChild(identity);row.appendChild(quantity);content.appendChild(row);});(instances||[]).forEach(function(item){const row=document.createElement('div');row.className='d-flex justify-content-between border-bottom py-2';const identity=document.createElement('span');identity.textContent=item.item_name+' +'+item.refinement_level;const reference=document.createElement('strong');reference.textContent='Ref. '+item.public_reference;row.appendChild(identity);row.appendChild(reference);content.appendChild(row);});}
    ...
    renderInventoryItems(initialInventory.stackable_items,initialInventory.item_instances,initialInventory.inventory_status);render(lastState);schedule(lastState.status==='running'?250:null);
})();
</script>
@endsection
